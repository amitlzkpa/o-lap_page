...
function updateGraph(jsonDataForGraph, graphSrc, msgArray) {
    ...
    if (Graph.isValidGraphData(jsonDataForGraph)) {
	    ...
	    olapViz.setGraph(graph);
	    reloadGraph(olapViz.getGraph());
...
beforeSceneInitialized = function() {
    setupKeyListeners();
}


afterSceneInitialized = function() {
    olapViz = new OLAPVisualization("olapViz");
    OLAP.addVisualization(olapViz);
    olapViz.registerListener(webHTML);
}


// called before frame is rendered;
beforeFrameUpdate = function() {
}


// called after frame is rendered;
afterFrameUpdate = function() {
}
